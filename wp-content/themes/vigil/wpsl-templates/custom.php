...
//$output .= "\t\t\t\t\t" . '<div><label for="wpsl-search-input">' . esc_html( $wpsl->i18n->get_translation( 'search_label', __( 'Your location', 'vigil' ) ) ) . '</label></div>' . "\r\n";
$output .= "\t\t\t\t\t" . '<input id="wpsl-search-input" type="text" value="' . apply_filters( 'wpsl_search_input', '' ) . '" name="wpsl-search-input" aria-required="true" placeholder="' . esc_html( $wpsl->i18n->get_translation( 'search_label', esc_html__( 'Location', 'vigil' ) ) ) . '" />' . "\r\n";
$output .= "\t\t\t\t\t" . '<div class="wpsl-search-btn-wrap"><input id="wpsl-search-btn" type="submit" value="&#xf002;" class="dt-sc-storesearch-btn"></div>' . "\r\n";

$output .= "\t\t\t\t" . '</div>' . "\r\n";

$output .= "\t\t\t" . '<a href="#" class="dt-sc-toggle-advanced-options">'.esc_html__('Advanced Options', 'vigil').' <span class="fa fa-angle-down"></span> </a>' . "\r\n";
	...
	if ( $wpsl_settings['radius_dropdown'] || $wpsl_settings['results_dropdown']  ) {
		...
		$output .= "\t\t\t" . '<div class="wpsl-select-wrap">' . "\r\n";
			...
			if ( $wpsl_settings['radius_dropdown'] ) {
				...
				$output .= "\t\t\t\t\t" . '<label for="wpsl-radius-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'radius_label', __( 'Search radius', 'vigil' ) ) ) . '</label>' . "\r\n";
			...
			if ( $wpsl_settings['results_dropdown'] ) {
				...
				$output .= "\t\t\t\t\t" . '<label for="wpsl-results-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'results_label', __( 'Results', 'vigil' ) ) ) . '</label>' . "\r\n";
...
if ( $wpsl_settings['show_credits'] ) { 
    $output .= "\t" . '<div class="wpsl-provided-by">'. sprintf( __( "Search provided by %sWP Store Locator%s", "vigil" ), "<a target='_blank' href='https://wpstorelocator.co'>", "</a>" ) .'</div>' . "\r\n";
}
