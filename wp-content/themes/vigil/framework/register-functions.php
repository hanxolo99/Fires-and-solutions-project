 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('vigil_features')) {

	function vigil_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'vigil-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'vigil-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'vigil-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'vigil-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'vigil-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'vigil-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list

			if(class_exists('DTRestaurantAddon')):
				add_image_size( 'vigil-menu-type2', 130, 130, true  ); // menu type2
				add_image_size( 'vigil-chef-type1', 420, 383, true  ); // chef type1
				add_image_size( 'vigil-chef-type2', 420, 420, true  ); // chef type2
			endif;

			if(class_exists('DTRoomAddon'))
				add_image_size( 'vigil-room-type', 400, 297, true  ); // room type
				
			if(class_exists('DTEventAddon')):
				add_image_size( 'vigil-events-grid', 375, 375, true  ); // events grid
				add_image_size( 'vigil-events-list', 420, 285, true  ); // events list
			endif;
			
			if(class_exists('DTProgramAddon')):
				add_image_size( 'vigil-training-list2', 280, 311, true  ); // training list2
			endif;
		}
	...
	add_action('after_setup_theme', 'vigil_features');
}

/* ---------------------------------------------------------------------------
 * Check Activate Theme
 * --------------------------------------------------------------------------- */
if (!function_exists('vigil_activation_function')) {
	function vigil_activation_function() {
		...
		if(!in_array( wp_get_theme()->get('Name'), array('VIGIL', 'VIGIL Child'))) {
			update_option( '_cs_options', $options );
		}
	}
	add_action("after_switch_theme", "vigil_activation_function", 10, 1 );
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('vigil_under_construction') ){
	function vigil_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'vigil_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'vigil_rand_css', 101);
	function vigil_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, VIGIL_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('vigil_wp_list_categories') ){
	function vigil_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'vigil_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('vigil_wp_list_archive') ){
	function vigil_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'vigil_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('vigil_wpcf7_form_elements') ){
	function vigil_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'vigil_wpcf7_form_elements');
}
 ...
 * --------------------------------------------------------------------------- */
function vigil_slider( $id = false ){
		...
		if( is_array( $slider_key) && $slider_key['slider_type'] != '' ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( $slider_key['revolutionslider_id'] != '' ) && ( $slider_key['show_slider'] == 'true' ) ){
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= vigil_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
function vigil_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'vigil') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'vigil'),
					'next_text'          => esc_html__('Next', 'vigil').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'vigil_page_title' ) ){
	function vigil_page_title( $echo = false ){
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
function vigil_breadcrumbs(){
	global $post;

	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.vigil_cs_get_option( 'breadcrumb-delimiter', 'fa default' ).'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.vigil_cs_get_option( 'breadcrumb-delimiter', 'fa default' ).'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','vigil'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  '<a href="'. $homeLink .'">'. esc_html__('Home','vigil') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'vigil').'</a>';

	} elseif( is_post_type_archive('dt_portfolios') ){

		$breadcrumbs[] = '<span class="current">'. esc_html__('Portfolios', 'vigil') .'</span>';

	} elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(vigil_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$cat = get_the_term_list(vigil_ID(), 'product_cat', '', '$$$', '');
			$cats = array_filter(explode('$$$', $cat));
			if (!empty($cats))
				$breadcrumbs[] = $cats[0];
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( vigil_ID() ) .'</span>';
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
function vigil_breadcrumb_section($id, $post_type ) {
	...
	if( !empty( $bcrumb ) ) :
		...
		$bstyle = vigil_cs_get_option( 'breadcrumb-style', 'default' );
...
function vigil_breadcrumb_section_with_class ( $title , $class ){
...
function vigil_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'vigil' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'vigil'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'vigil' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'vigil' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'vigil' );
		}
		...
		$title = esc_html__('Tag Archives','vigil');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function vigil_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function vigil_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('vigil_hex2rgb')) {

	function vigil_hex2rgb($hex) {
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
function vigil_header_logo() {
	echo '<div id="logo">';

        $use_logo = (int) get_theme_mod( 'use-custom-logo', vigil_defaults('use-custom-logo') );

		if( !empty( $use_logo ) ):

			$url 		= get_theme_mod( 'custom-logo', vigil_defaults('custom-logo') );
			$darkbg_url = get_theme_mod( 'custom-dark-logo', vigil_defaults('custom-dark-logo') );?>
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function vigil_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'vigil');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'vigil'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'vigil');
					endif;
					edit_comment_link(esc_html__('Edit', 'vigil'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'vigil'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
function vigil_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function vigil_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function vigil_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $vigil_like_love;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'vigil_like_love':
			return $vigil_like_love;
		break;
	}
	return false;
}
 ...
 * break oembeds.
 * --------------------------------------------------------------------------- */
function vigil_undo_recurrence_oembed_logic() {
...
add_action( 'init', 'vigil_undo_recurrence_oembed_logic' ); 

/* ---------------------------------------------------------------------------
 * string rotate
 * --------------------------------------------------------------------------- */
function vigil_str_rot($s, $n = 13) {
