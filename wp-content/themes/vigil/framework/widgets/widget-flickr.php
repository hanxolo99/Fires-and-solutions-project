<?php 
class VIGIL_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>esc_html__('A widget that show last flickr photo streams', 'vigil'));
		parent::__construct(false,VIGIL_THEME_NAME.esc_html__(' Flickr Widget','vigil'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','vigil');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','vigil');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','vigil'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('show')); ?>" name="<?php echo ($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=>esc_html__("Latest",'vigil'),"random"=>esc_html__("Random",'vigil'));
         ...
         <p><label for="<?php echo ($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','vigil'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('size')); ?>" name="<?php echo ($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=>esc_html__("square",'vigil'),"t"=>esc_html__("thumbnail",'vigil'), "m" => esc_html__("medium",'vigil'));
			foreach($a as $key => $value ):
