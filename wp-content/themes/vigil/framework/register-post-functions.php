...
class vigil_post_functions {

	function __construct() {

		add_action( 'wp_ajax_vigil_post_rating_like', array( $this, 'vigil_post_rating_like' ) );
		add_action( 'wp_ajax_nopriv_vigil_post_rating_like', array( $this, 'vigil_post_rating_like' ) );

	}

	function vigil_post_meta_fields( $ajax = false, $page_ID = 0 ) {
	...
	static function vigil_post_view_count($pid = '') {
	...
	static function vigil_post_like_count($pid = '') {
	...
	function vigil_post_rating_like() {
		...
		if ( wp_verify_nonce( $nonce, 'rating-nonce' ) && $postid > 0 ) {
			...
			$out = esc_html__('Security check', 'vigil');
		}
	...
	function vigil_post_thumb( $post_ID ) {
		...
		elseif( $format === "audio" ) :
			if( has_post_thumbnail($post_ID) ) : ?>
				<a href="<?php echo get_permalink($post_ID);?>" title="<?php printf(esc_attr__('Permalink to %s','vigil'), get_the_title($post_ID));?>">
	...
	function vigil_post_default_style( $post_ID, $meta ) {
        ...
        <div class="entry-thumb">
        	<?php self::vigil_post_thumb( $post_ID ); ?>
        ...
        <div class="entry-details">
            ...
            <div class="entry-meta <?php echo esc_attr($tclass);?>">
            	<div class="date <?php echo esc_attr($meta[6]);?>"><?php esc_html_e('Posted on ','vigil'); echo get_the_date ( get_option('date_format') );?></div>
                ...
                <div class="author <?php echo esc_attr( $meta[5] );?>">
                    ...
                    <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                        title="<?php esc_attr_e('View all posts by ', 'vigil'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                </div>
            </div><!-- Meta -->

            <div class="entry-title">
                <?php if( is_sticky( $post_ID ) ) echo '<span class="sticky-post">'.esc_html__('Featured', 'vigil').'</span>'; ?>
                <h4><a href="<?php echo get_permalink( $post_ID );?>" title="<?php printf(esc_attr__('Permalink to %s','vigil'), the_title_attribute('echo=0'));?>"><?php the_title();?></a></h4>
            </div>

			<?php if( isset($meta[2]) && isset($meta[3]) ):?>
				<div class="entry-body"><?php echo vigil_excerpt($meta[3]);?></div>
            <?php endif;?>
            ...
            <!-- Read More Button -->
            <?php 
			if( vigil_is_plugin_active('designthemes-core-features/designthemes-core-features.php') && class_exists( 'DTCorePlugin' ) ){
				if( isset($meta[0]) ):
				...
				<a href="<?php echo get_permalink( $post_ID ); ?>" class="dt-sc-button small icon-right with-icon filled type1"><?php _e("Read More","vigil"); ?> <span class="fa fa-long-arrow-right"> </span></a><?php
			}?><!-- Read More Button -->

        </div><!-- Post Details --><?php
	}

	function vigil_post_date_left_style( $post_ID, $meta ) {
	...
	function vigil_post_date_author_left_style( $post_ID, $meta ) {
        ...
        <div class="entry-date-author <?php echo esc_attr($tclass);?>">
            ...
            <div class="entry-author <?php echo esc_attr( $meta[5] );?>">
                ...
                <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                    title="<?php esc_attr_e('View all posts by ', 'vigil'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
            </div>
